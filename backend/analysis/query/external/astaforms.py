...
scores = [['', 'Score', 'SD'],
          ...
          ['Aantal lexicale werkwoorden', "='ZNW & WW'!B1", applytemplate2(4)],
...
sheet2header = ['Nummer', 'Zelfstandig naamwoord',	'Herhaling',
                'Aantal', '',	'Lexicaal werkwoord', 'Herhaling', 'Aantal']
sheet2colwidths = [10, 25, 20, 10, 5, 25, 20, 10]
sheet3header = ['Uitingsnummer', 'Aantal woorden', 'Correct',	"Goede PV's",
                "Foute en ontbrekende PV's", "Aantal bijzinnen",	"Bijzonderheden"]
...
def make_astaform(theform, astadata, outfullname):
...
def getvardict(allresults):
    ...
    KM = 'B9'
    semparcount = sumctr(
        allresults.coreresults['A026']) if 'A026' in allresults.coreresults else 0
    phonparcount = sumctr(
        allresults.coreresults['A008']) if 'A008' in allresults.coreresults else 0
    neocount = sumctr(
        allresults.coreresults['A022']) if 'A022' in allresults.coreresults else 0
    Kcount = sumctr(
