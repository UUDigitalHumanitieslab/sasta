...
expected = {
    1: {
        'treeyield': ['hij', 'wil', 'niet', 'buiten', '.'],
        'resultyield': []
    },
    2: {
        'treeyield': ['emmer', 'mee', 'voor', 'de', 'varkens', 'en', 'voor', 'de', 'poesjes', '.'],
        'resultyield': ['en']
    },
    3: {
        'treeyield': ['water', 'en', 'melk', '.'],
        'resultyield': ['en']
    },
}
...
def test_xenx():
    ...
    for i in testtrees:
        ...
        # print(treeyield)
        assert expected[i]['treeyield'] == treeyield
        for result in results:
            resultyield = getyield(result)
            # print('--', resultyield)
            assert expected[i]['resultyield'] == resultyield
