...
def activate_frontend():
    ...
    if framework == 'backbone':
        ...
        project_name = 'sasta'.replace('_', '-')
        Command(
            'Creating project',
            ['npx', '-p', ANGULAR_CLI, 'ng', 'new', project_name, '--prefix=sas',
                '--skipGit=true',
                '--skipInstall=true',
                '--style=scss']
        )()
        ...
        if '4200' != '4200':
            Command(
                'Set frontend port',
                ['ng', 'config', 'projects.sasta.architect.serve.options.port', '4200'],
                cwd="frontend"
            )()
    else:
